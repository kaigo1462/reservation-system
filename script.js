...
form.addEventListener("submit", async (event) => {
  ...
  if (!ALLOWED_AREAS.includes(area)) {
    message.textContent = "居住地区を選択してください。";
    areaInput.focus();
    return;
  }
  if (!/^[0-9]+$/.test(phone)) {
  ...
  if (error) {
    ...
    const errorText = [error.message || "", error.details || "", error.hint || ""].join(" ");
    if (errorText.includes("RESERVATION_LIMIT_REACHED")) {
      message.textContent = "予約が定員150名に達したため、受付を終了しました。";
      submitButton.disabled = true;
    } else {
  ...
  document.getElementById("ticket-area").textContent = area;
...
pdfButton.addEventListener("click", async () => {
  ...
  try {
    ...
    else pdf.save(currentReservationNumber + ".pdf");
